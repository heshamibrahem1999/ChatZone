    ...
    <div class="invite-card">
        ...
        <div class="invite-link">
            ...
            <button class="btn" type="button"
                onclick="navigator.clipboard.writeText(document.getElementById('inviteLink').value); this.textContent='Copied';">Copy</button>
        </div>
        <?php if ($isAdmin): ?>
        <div class="actions">
            <form method="post"><input type="hidden" name="group_id" value="<?= $groupId ?>"><input type="hidden"
                    name="action" value="reset"><button class="btn secondary"
                    onclick="return confirm('Reset invite link? Old link will stop working.')">Reset link</button>
            </form>
            <?php if ((int)($group['invite_enabled'] ?? 1) === 1): ?>
            <form method="post"><input type="hidden" name="group_id" value="<?= $groupId ?>"><input type="hidden"
                    name="action" value="disable"><button class="btn danger">Disable link</button></form>
            <?php else: ?>
            <form method="post"><input type="hidden" name="group_id" value="<?= $groupId ?>"><input type="hidden"
                    name="action" value="enable"><button class="btn">Enable link</button></form>
            <?php endif; ?>
        </div>
        <?php endif; ?>
    </div>
