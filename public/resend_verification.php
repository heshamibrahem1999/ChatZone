...
<head>
    ...
    <title>Resend Verification - ChatZone</title>
    <link rel="stylesheet" href="assets/css/auth.css">
    <link rel="stylesheet" href="assets/css/extracted/public__resend_verification.css">
</head>
...
<body>
    <main class="auth-page">
        <section class="auth-card">
            ...
            <h1 class="auth-title">Resend verification</h1><?php if ($message): ?><div
                class="alert <?= $link ? '' : 'error' ?>"><?= htmlspecialchars($message, ENT_QUOTES, 'UTF-8') ?></div>
            <?php endif; ?><form method="post">
                <div class="form-group"><label>Email</label><input type="email" name="email"
                        value="<?= htmlspecialchars($email, ENT_QUOTES, 'UTF-8') ?>" required></div><button
                    class="btn-primary">Send link</button>
            </form><?php if ($link): ?><div class="small-link"><a
