...
<body>
    <main class="wrap">
        <div class="top">
            <div>
                <h1>🧾 Activity Logs</h1>
                <p class="muted">Admin audit trail for important actions.</p>
            </div>
            <div><a class="btn" href="admin_dashboard.php">📊 Dashboard</a> <a class="btn" href="chat.php">← Chat</a>
            </div>
        </div>
        <div class="card">
            <form class="filters" method="get">
                ...
                <select name="action">
                    <option value="">All actions</option><?php foreach ($actions as $a): ?><option
                        value="<?= e($a['action']) ?>" <?= $action===$a['action']?'selected':'' ?>>
                        <?= e($a['action']) ?> (<?= (int)$a['c'] ?>)</option><?php endforeach; ?>
                </select>
                <button class="btn" type="submit">Filter</button><a class="btn btn-light"
                    href="admin_activity.php">Reset</a>
            </form>
        </div>
        <div class="card">
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?= $l['user_id'] ? '<b>'.e(trim(($l['first_name'] ?? '').' '.($l['last_name'] ?? ''))).'</b><br><span class="muted">'.e($l['email'] ?? ('ID '.$l['user_id'])).'</span>' : '<span class="muted">System/Guest</span>' ?>
                        </td>
                        <td><span class="badge"><?= e($l['action']) ?></span></td>
                        <td class="hide-sm">
                            <?= e((string)($l['target_type'] ?? '')) ?><?= $l['target_id'] ? ' #'.(int)$l['target_id'] : '' ?>
                        </td>
        ...
        <div class="pager"><?php if($page>1): ?><a class="btn btn-light"
                href="?page=<?= $page-1 ?>&action=<?= urlencode($action) ?>&q=<?= urlencode($q) ?>">Previous</a><?php endif; ?><?php if($page<$pages): ?><a
                class="btn"
                href="?page=<?= $page+1 ?>&action=<?= urlencode($action) ?>&q=<?= urlencode($q) ?>">Next</a><?php endif; ?>
        </div>
    </main>
</body>
