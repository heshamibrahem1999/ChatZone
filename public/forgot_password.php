...
?>
<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Forgot Password - ChatZone</title><link rel="stylesheet" href="assets/css/auth.css"><link rel="stylesheet" href="assets/css/extracted/public__forgot_password.css"></head><body><main class="auth-page"><section class="auth-card"><div class="auth-logo">🔑</div><h1 class="auth-title">Forgot password</h1><?php if ($message): ?><div class="alert"><?= htmlspecialchars($message, ENT_QUOTES, 'UTF-8') ?></div><?php endif; ?><form method="post"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars(csrf_token(), ENT_QUOTES, 'UTF-8') ?>"><div class="form-group"><label>Email</label><input type="email" name="email" required></div><button class="btn-primary">Generate reset link</button></form><?php if ($link): ?><p>Local testing link:</p><div class="small-link"><a href="<?= htmlspecialchars($link, ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars($link, ENT_QUOTES, 'UTF-8') ?></a></div><?php endif; ?><div class="auth-link"><a href="index.php">Back to login</a></div></section></main></body></html>
